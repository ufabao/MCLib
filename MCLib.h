...
template <typename T> struct SampleDef {
  bool numeraire = true;
  std::vector<double> forward_maturities;
  std::vector<double> discount_maturities;
};

template <typename T> struct MarketSample {
  T numeraire{T(1.0)};
...
template <typename T> class Instrument {
public:
...
template <typename T> class FinancialModel {
public:
...
// the water
// In order to be compatible with Sobol and some other deterministic PRNG's,
// we require our RNG interface to have a jump ahead this has the added
// benefit that a parallel simulation and non parallel simulation with the
// same seed will have the same result.
class RNG {
public:
...
// (hopefully) shine!
{
  // First some prep work. Declare a batch size, which will be the number of
  // paths generated in each iteration. We copy the RNG, each thread needs
  // ownership of one RNG so we will need to copy it for at least all but the
  // main thread, so why not just copy for the main thread too and end the
  // simulation with the same RNG ready to go. The we allocate the result
  // matrix. finally we attune the model to the instrument -- this tells the
  // model what it needs to simulate
  size_t batch_size = 256;
  auto crng = rng.clone();
  const auto number_of_payoffs = instrument.number_of_payoffs();
  std::vector<std::vector<double>> results(
      number_of_iterations, std::vector<double>(number_of_payoffs));
  model.attune(instrument);
  ...
  for (auto &vec : gaussian_matrix)
    vec.resize(model.simulation_dimension());
  ...
  // in each iteration the producer will increment the first_path in the batch,
  // starting with 0, and decrement the number of paths left
  size_t first_path = 0;
  size_t paths_left = number_of_iterations;

  while (paths_left > 0) {
    // all but the last batch will receive batch_size paths to compute
    // the producer bundles up a nice bit of work and kicks it to the threadpool
    // in a lambda, the spawn_task function returns a future<bool> for that
    // work, which is pushed into our futures_vector
    ...
    future_vector.push_back(pool->spawnTask(
        ...
        {
          const size_t thread_num = pool->threadNum();
